...
// as eth_sendTransaction to the hardhat fork, which signs for its own
// unlocked accounts. Configure it from the console before reloading:
//   localStorage.setItem("arcadepad:dev-wallet", JSON.stringify({ rpc: "http://127.0.0.1:8697", address: "0x7099…" }))
(function () {
  var raw = null;
  try {
    raw = localStorage.getItem("arcadepad:dev-wallet");
  } catch {}
  if (!raw) return;
  ...
  console.log("[arcadepad] dev wallet provider installed for", cfg.address);
})();
