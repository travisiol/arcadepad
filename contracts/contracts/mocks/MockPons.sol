...
import {IPonsFactoryV2, IPonsFeeEscrow, IPonsLaunchForwarder} from "../interfaces/IPonsV2.sol";

/**
 * Just enough Pons to unit-test the pad: a factory that mints a token and
 * a constant-product curve (native or ERC-20 quote), a forwarder that
 * launches then buys, and a fee escrow that can be credited by hand (on
 * the real chain only Pons sweeps fees into it). The real thing is
 * exercised on a fork (scripts/fork-check.ts); this mock only checks the
 * pad's own logic.
 */
contract MockLaunchedToken is ERC20 {
    /// Pons tokens keep their launch metadata on chain; the site reads it.
    string public logo;
    ...
    string[5] private _socials;

    function setMeta(string memory logo_, string memory description_, address deployer_, IPonsFactoryV2.Socials memory s) external {
        logo = logo_;
        description = description_;
        deployer = deployer_;
        _socials = [s.x, s.telegram, s.website, s.discord, s.extra];
    }

    function socials() external view returns (string memory, string memory, string memory, string memory, string memory) {
        return (_socials[0], _socials[1], _socials[2], _socials[3], _socials[4]);
    }
}

/// A stand-in for a Robinhood stock token (GLD, SLV).
...
contract MockCurve {
    ...
    /// The real curve taxes buys 99 % for three seconds after launch; the mock never does.
    function currentSnipeTaxBps(address) external pure returns (uint256) {
        return 0;
    }

    /// Pons' share of the trade fee on the real curve.
    ...
    /// the way in. Native curve: quoteAmount == msg.value. ERC-20 curve:
    /// msg.value == 0 and quoteAmount is pulled from the caller.
...
/// Pons' fee escrow, as far as a recipient can see it: a balance and claim().
contract MockFeeEscrow is IPonsFeeEscrow {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _tokenBalances;

    /// Test hook: what Pons' sweep would do — credit a recipient.
    ...
    function claim() external override {
        ...
        require(amount > 0, "escrow: nothing");
        _balances[msg.sender] = 0;
        (bool ok, ) = msg.sender.call{value: amount}("");
        require(ok, "escrow: pay");
    }

    function claimToken(address) external pure override {
        revert("escrow: no token fees in the mock");
    }
}
...
contract MockPonsFactory is IPonsFactoryV2 {
    ...
    constructor(address feeSink_) {
        ...
        feeEscrow = address(new MockFeeEscrow());
    }
    ...
    function launchToken(
        ...
        t.setMeta(params.logo, params.description, params.creatorFeeRecipient, params.socials);
